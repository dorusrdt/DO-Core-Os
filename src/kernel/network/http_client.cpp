...
HttpClientManager::HttpClientManager() {
    ...
    strcpy(config.server_url, "http://192.168.1.72");
    config.port = 8000;
...
HttpClientError_t HttpClientManager::send_request_internal(HttpRequest_t* request, HttpResponse_t* response) {
    ...
    int http_code = 0;
    bool begun = client.begin(full_url);
    if (!begun) {
        response->response_time_ms = millis() - start_time;
        response->success = false;
        response->error_code = HTTP_CLIENT_ERROR_CONNECTION;
        snprintf(response->error_message, sizeof(response->error_message), "begin() failed for %s", full_url);
        kernel_log(LOG_LEVEL_ERROR, "HTTP begin() failed: %s", full_url);
        client.end();
        return HTTP_CLIENT_ERROR_CONNECTION;
    }
    ...
    if (http_code >= 200 && http_code < 300) {
        ...
        // Récupérer un header utile (Content-Type)
        ...
        response->success = false;
        response->error_code = (http_code == 0) ? HTTP_CLIENT_ERROR_CONNECTION : HTTP_CLIENT_ERROR_RESPONSE;
        snprintf(response->error_message, sizeof(response->error_message), "HTTP status: %d", http_code);
        kernel_log(LOG_LEVEL_WARN, "HTTP non-success status: %d", http_code);
    }
