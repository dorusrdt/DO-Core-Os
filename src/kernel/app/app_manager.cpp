...
// Obtenir l'ID d'une application par son nom
uint8_t app_get_id_by_name(const char* name) {
    if (!app_manager_initialized || !name) {
        return 0xFF;  // Valeur invalide
    }
    ...
    for (int i = 0; i < MAX_APPS; i++) {
        if (apps[i].info.app_id != 0 && strcmp(apps[i].info.name, name) == 0) {
            uint8_t app_id = apps[i].info.app_id;
            xSemaphoreGive(app_manager_mutex);
            return app_id;
        }
    }
