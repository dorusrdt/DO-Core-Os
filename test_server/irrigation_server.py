...
Simulates the external server (192.168.1.72:3000)
...
# ============================================================================
# Zone Management Endpoints (NEW - for create_test_zones.sh)
# ============================================================================
...
async def create_zone(device_id: str, zone_data: Dict[str, Any]):
    ...
    existing_zone = next((z for z in config["zones"] if z.get("zoneId") == zone_data.get("zoneId")), None)
    ...
    print(f"[{datetime.now().isoformat()}] Zone {action}: {zone_data.get('zoneId')} for {device_id}")
    print(f"  Physical Zone: {zone_data.get('physicalZoneNumber')}")
    print(f"  Total zones: {len(config['zones'])}")
...
async def get_zones(device_id: str):
    ...
    print(f"[{datetime.now().isoformat()}] Getting zones for {device_id}: {len(zones)} zones")
...
if __name__ == "__main__":
    ...
    print("  POST   /api/devices/{id}/zones        - Create/add zone (NEW)")
    print("  GET    /api/devices/{id}/zones        - List zones (NEW)")
